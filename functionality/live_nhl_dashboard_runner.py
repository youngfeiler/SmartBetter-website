...
class live_nhl_dashboard_runner():
    ...
    def convert_times_to_est(self, df):
      # Actually converts it to EST when we're in daylight savings time, but as long as we're not in GMT we're chilling
    ...
    def add_extra_info(self, df):
      ...
      extra_info = pd.read_csv('nhl_data/nhl_extra_info.csv')

      extra_info['date'] = pd.to_datetime(extra_info['date'])

      df['commence_time'] = pd.to_datetime(df['commence_time'])

      extra_info['day_of_week'] = extra_info['date'].dt.strftime('%a')

      extra_info['date'] = extra_info['date'].dt.strftime('%m/%d/%Y')

      df['commence_time'] = df['commence_time']

      df['time'] = df['commence_time'].dt.strftime('%I:%M:%S %p')

      compare_time = pd.to_datetime('17:00:00').time()

      df['compare_time'] = pd.to_datetime(df['time']).dt.time

      df['day_night'] = np.where(df['compare_time'] > compare_time, 'N', 'D')
      ...
      n_count = merged_df['day_night'].value_counts().get('N', 0)
      d_count = merged_df['day_night'].value_counts().get('D', 0)

      merged_df = merged_df.dropna(subset=['team_1'])

      merged_df = merged_df.drop(columns=['commence_date', 'date', 'time','team_1_df2', 'team_2_df2', 'commence_date_df2'])
      ...
      merged_df['commence_time'] = merged_df['commence_time']
      merged_df['snapshot_time'] = merged_df['snapshot_time']
    ...
    def stack_df(self, df):
      ...
      extra_cols = ['game_id', 'commence_time', 'time_pulled', 'my_game_id', 'hour_of_start', 'day_of_week', 'winning_team', 'home_away_neutral', 'home_team', 'away_team',  'home_team_division', 'away_team_division', 'team_1_division', 'team_2_division', 'home_team_conference', 'away_team_conference', 'day_night', 'snapshot_time', 'minutes_since_commence']
    ...
    def make_some_features(self, df):
       ...
       def make_highest_bettable_odds(df):
        subset_columns = [col for col in df.columns if any(item in col for item in self.model_storage['SmartBetterNHLModel']['params']['bettable_books'])]
        ...
        threshold = self.model_storage['SmartBetterNHLModel']['params']['update_time_threshold']
    ...
    def filter_by_params(self, df):
       ...
       def filter_by_average_market_odds(df):
        df = df[df['average_market_odds_old'] >= self.model_storage['SmartBetterNHLModel']['params']['min_avg_odds']]
        df = df[df['average_market_odds_old'] <= self.model_storage['SmartBetterNHLModel']['params']['max_avg_odds']]
        return df
       ...
       def filter_by_best_odds(df):
          df = df[df['highest_bettable_odds'] >= self.model_storage['SmartBetterNHLModel']['params']['min_avg_odds']]
          df = df[df['highest_bettable_odds'] <= self.model_storage['SmartBetterNHLModel']['params']['max_avg_odds']]
    ...
    def format_for_nn(self):
       def drop_unnecesary_columns(df):
        ...
        count_dict = Counter(df.columns.tolist())
       ...
       def standardize_numerical_values(df):
        ...
        scaler = self.model_storage['SmartBetterNHLModel']['scaler']
       ...
       def encode_categorical_variables(df):
        ...
        encoder_obj = self.model_storage['SmartBetterNHLModel']['encoder']
      ...
      if not self.filtered_df.empty:
        for strategy_name, strategy_dict in self.model_storage.items():
          ...
          if not filtered_df.empty:
            ...
            filtered_df['team_1_division'] = test_series.iloc[:, 0]
