...
class PositiveEVDashboardRunner():
  ...
  def handle_positive_ev_observations(self, df):
     ...
     full_df = pd.read_csv('pos_ev_data/nhl_pos_ev_data.csv')
     ...
     result.to_csv('pos_ev_data/nhl_pos_ev_data.csv', index=False)
 
  def remove_event_obs(self, game_id):
   df = pd.read_csv("pos_ev_data/nhl_pos_ev_data.csv")
   df = df[df['game_id'] != game_id]
   df.to_csv("pos_ev_data/nhl_pos_ev_data.csv", index= False)
   return 
