...
class pregame_nba_dashboard_runner():
    def __init__(self):
        self.model_storage = {}
    ...
    def store_model_info(self):
          loaded_model = torch.load(f'models/model_objs/nba_model_pregame.pth')
          with open(f'models/encoders/nba_model_pregame.pkl', 'rb') as f:
            loaded_encoder = pickle.load(f)
          with open(f'models/scalers/nba_model_pregame.pkl', 'rb') as f:
            loaded_scaler = pickle.load(f)
          with open(f'models/params/nba_model_pregame.pkl', 'rb') as f:
          ...
          try:
            ...
            this_model_dict['params']['bettable_books'].remove('betonlineag')
             ...
             pass
          self.model_storage['SmartBetterNBAModel'] = this_model_dict
    ...
    def make_snapshot(self, df):
      ...
      #TODO: make sure at some point before preprocessing we subtract 4 hours from every time column
      for game in df:
    ...
    def add_extra_info(self, df):
      ...
      extra_info = pd.read_csv('nba_data/nba_extra_info.csv')
      ...
      time_columns.remove('compare_time')
    ...
    def make_some_features(self, df):
       ...
       def make_highest_bettable_odds(df):
        subset_columns = [col for col in df.columns if any(item in col for item in self.model_storage['SmartBetterNBAModel']['params']['bettable_books'])]
    ...
    def filter_by_params(self, df):
       ...
       def filter_by_minutes_since_commence(df):
          df = df[df['minutes_since_commence'] >= self.model_storage['SmartBetterNBAModel']['params']['min_minutes_since_commence']]
          df = df[df['minutes_since_commence'] <= self.model_storage['SmartBetterNBAModel']['params']['max_minutes_since_commence']]

          return df
       
       def filter_by_average_market_odds(df):
        df = df[df['average_market_odds_old'] >= self.model_storage['SmartBetterNBAModel']['params']['min_avg_odds']]
        df = df[df['average_market_odds_old'] <= self.model_storage['SmartBetterNBAModel']['params']['max_avg_odds']]
        return df
      ...
      if not self.filtered_df.empty:
        for strategy_name, strategy_dict in self.model_storage.items():
          ...
          if not filtered_df.empty:
            ...
            print(f"{len(filtered_df)} NBA pregame bets found" )
